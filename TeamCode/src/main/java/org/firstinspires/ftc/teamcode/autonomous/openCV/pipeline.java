...
public class pipeline extends OpenCvPipeline {
	...
	public Mat processFrame(final Mat input) {
		...
		Imgproc.rectangle(output, CENTER_RECT,
				isRed ? new Scalar(255d, 0d, 0d) : new Scalar(0d, 0d, 255d));
		Imgproc.rectangle(output, LEFT_RECT,
				isRed ? new Scalar(255d, 0d, 0d) : new Scalar(0d, 0d, 255d));
	...
	public void setLocation() {
		...
		else if (leftConfidence > centerConfidence && leftConfidence > CONFIDENCE) location = 1;
		else location = 3;
	}
}